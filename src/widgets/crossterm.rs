...
impl<'a, W: Output<TUI<'a>, (u16, u16)>> Output<TUI<'a>, (u16, u16)> for Fixed<u16, W> {
    fn render (&self, context: &mut TUI<'a>) -> Result<Option<(u16, u16)>> {
...
impl<'a, T> Output<TUI<'a>, (u16, u16)> for Max<u16, T>
where
    T: Output<TUI<'a>, (u16, u16)>
{
    fn render (&self, context: &mut TUI<'a>) -> Result<Option<(u16, u16)>> {
        self.get().render(context.area(|area|match self {
            Self::X(max_width, _) => {
...
impl<'a, T> Output<TUI<'a>, (u16, u16)> for Min<u16, T>
where
    T: Output<TUI<'a>, (u16, u16)>
{
    fn render (&self, context: &mut TUI<'a>) -> Result<Option<(u16, u16)>> {
...
impl<'a, T> Output<TUI<'a>, (u16, u16)> for Offset<u16, T>
where
    T: Output<TUI<'a>, (u16, u16)>
{
    fn render (&self, context: &mut TUI<'a>) -> Result<Option<(u16, u16)>> {
...
impl<'a> Output<TUI<'a>, (u16, u16)> for Stacked<'a, TUI<'a>, (u16, u16)> {
    fn render (&self, context: &mut TUI<'a>) -> Result<Option<(u16, u16)>> {
        let mut x = 0;
        let mut y = 0;
        match self.0 {
...
impl<'a> Stacked<'a, TUI<'a>, (u16, u16)> {
    /// Return an error if this area is larger than the minimum needed size
    pub fn expect_min (&self, area: &impl Rect<u16>, (min_w, min_h): (u16, u16)) -> std::io::Result<&Self> {
...
mod test {
    ...
    impl<'a> Output<TUI<'a>, (u16, u16)> for StackedWidget {
